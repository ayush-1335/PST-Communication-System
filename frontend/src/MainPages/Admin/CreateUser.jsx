        ...
        <div className="mb-6 space-y-3">
          {result.success.length > 0 && (
            <div className="px-4 py-3 rounded-lg bg-green-50 border border-green-200">
              <p className="text-sm font-semibold text-green-700 mb-2">✓ Created ({result.success.length})</p>
              <div className="space-y-1">
                {result.success.map((u, i) => (
                  <div key={i} className="flex items-center gap-2 text-sm text-green-700">
                    ...
                    <span className="ml-auto px-2 py-0.5 rounded-md bg-green-100 border border-green-200 text-green-600 text-xs font-medium">{u.role}</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-1">
                {result.failed.map((u, i) => (
                  <div key={i} className="flex items-center gap-2 text-sm text-red-600">
                    ...
                    <span className="text-red-500">{u.reason}</span>
                  </div>
                ))}
              </div>
            </div>
